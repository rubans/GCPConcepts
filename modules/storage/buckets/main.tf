...
locals {
  prefix = var.prefix == "" ? "" : join("-", list(var.prefix, lower(var.region), ""))
  folder_list = flatten([
...
resource "google_storage_bucket_object" "folders" {
  ...
  content  = "data"                  # Note that the content string isn't actually used, but is only there since the resource requires it
}
