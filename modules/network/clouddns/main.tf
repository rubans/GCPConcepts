...
locals {
  ...
  managed_zone_ref          = "${var.type != "public" ? "private" : "public"}-zone"
}
# implementation
locals {
...
resource "google_dns_managed_zone" "non-public" {
  ...
  project     = var.project
  name        = format(local.resource_name,local.managed_zone_ref)
  dns_name    = var.domain
...
resource "google_dns_record_set" "cloud-static-records" {
  ...
  project      = var.project
  managed_zone = format(local.resource_name,local.managed_zone_ref)
